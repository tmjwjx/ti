 ...
 * 通过 ANTHROPIC_BASE_URL 兼容任意 Anthropic Messages API 端点。
 *
 * 文件结构（自顶向下）：
 *   配置与鉴权 → 类型定义 → 系统提示词 → 工具定义/实现 → SSE 流式调用 → agent 循环 → REPL → 入口
 */
...
// ================================================================ 配置与鉴权
// API 端点：默认官方 api.anthropic.com；ANTHROPIC_BASE_URL 可切到任意兼容端点（如 Kimi coding）
const BASE_URL = (process.env.ANTHROPIC_BASE_URL ?? "https://api.anthropic.com").replace(/\/+$/, "");
// 两种鉴权方式（与 Claude Code 的习惯一致）：
//   ANTHROPIC_AUTH_TOKEN → 请求头 Authorization: Bearer <token>
//   ANTHROPIC_API_KEY    → 请求头 x-api-key: <key>
...
// -------- 命令行参数解析
// -m/--model 指定模型；-p/--prompt 进入单发模式；其余位置参数拼接成 prompt
let MODEL = process.env.ANTHROPIC_MODEL ?? "k3";
let prompt: string | undefined;
const rest: string[] = [];
const args = process.argv.slice(2);
for (let i = 0; i < args.length; i++) {
  const a = args[i];
  if ((a === "-m" || a === "--model") && args[i + 1]) MODEL = args[++i];
...
usage: node agent.ts [-m model] [-p prompt | prompt words...]
  -p, --prompt   run a single prompt non-interactively (default: interactive REPL)
  -m, --model    model name (default: $ANTHROPIC_MODEL or "k3")
env:  ANTHROPIC_BASE_URL, plus ANTHROPIC_AUTH_TOKEN or ANTHROPIC_API_KEY`);
  process.exit(code);
}
if (!AUTH_TOKEN && !API_KEY) {
  console.error("error: set ANTHROPIC_AUTH_TOKEN or ANTHROPIC_API_KEY");
  process.exit(1);
}

// ================================================================= 类型定义
// 对齐 Anthropic Messages API 的 content block（内容块）结构。
// 一条 assistant 消息的内容是块数组：文本块 + 工具调用块交错；
// 工具结果以 role:"user" 的消息回灌，内容是 tool_result 块数组。
...
// ================================================================ 工具定义
// Anthropic 工具定义格式：name + description + input_schema(JSON Schema)。
// 4 个工具的参数名与描述逐一对齐 pi 的 read/write/edit/bash，
// 模型对工具的用法预期因此与 pi 保持一致。
const TOOLS = [
  {
...
/**
 * 调用 Anthropic Messages API（stream:true）并手动解析 SSE 事件流。
 ...
 */
async function callLLM(messages: Message[], onText: (delta: string) => void): Promise<LlmResult> {
  const res = await fetch(`${BASE_URL}/v1/messages`, {
    method: "POST",
    headers: {
      "content-type": "application/json",
      "anthropic-version": "2023-06-01",
      ...(AUTH_TOKEN ? { authorization: `Bearer ${AUTH_TOKEN}` } : { "x-api-key": API_KEY! }),
    },
    body: JSON.stringify({ model: MODEL, max_tokens: 16384, stream: true, system: SYSTEM_PROMPT, messages, tools: TOOLS }),
  });
  ...
  /** 处理单个 SSE data 载荷（Anthropic 流式事件） */
  const handle = (ev: any) => {
    switch (ev.type) {
...
async function agentTurn(messages: Message[]): Promise<void> {
  for (let turn = 0; ; turn++) {
    ...
    // tool_result 以 role:"user" 消息回灌（Anthropic 协议约定），然后继续循环
...
async function repl(messages: Message[]): Promise<void> {
  ...
  rl.setPrompt(cyan("\n> "));
  console.log(dim(`ti — minimal pi-style agent | model: ${MODEL} | ${process.cwd()}`));
  console.log(dim("/clear to reset context, /exit to quit"));
  let closed = false;
  rl.once("close", () => {
