...
| **ti update** | 已落地 | `ti update` 把全局命令更新到官方源上查到的版本，按当时的安装器来装。改不了就只打印命令。直跑源码时拒绝（见 `docs/impl/update.md`） | 已安装的命令能升到官方源最新版；改不了时不安装；`npm start` 下报错退出 |
...
发布流程（届时）：仓库转公开 → `npm login` → `npm publish --access public`（scoped 首次必须带）→ git tag。当前已能 `npm pack` 或全局安装；打包还缺英文 README。
...
当前已落地：session、`/compact`、中断、输入体验、skills、token 累计、`/provider`、TUI、测试、打包（含 README、`--version`、打包检查）、`ti update`。权限确认不做。不做 CI。
