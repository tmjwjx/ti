...
**非目标（YAGNI，明确不做）**：插件/扩展系统、skills 市场、Web UI、MCP、子 agent、PTC 模式、主题系统、内置 todo 工具（用 TODO.md 文件代替，pi 哲学）。
...
| F3 | **权限模式** | mutation 类工具（write/edit/bash）执行前终端确认：`y` 本次允许 / `a` 本会话同类放行 / `n` 拒绝（以错误回灌模型）；`read` 永远免确认；`-y/--yes` 或配置 `permissions:"auto"` 全自动；非 TTY（管道）无法提问时按拒绝处理并提示 | 默认模式下 write/bash 必先弹确认；`-y` 全程无确认；拒绝后模型收到权限错误并调整 |
...
extensions（`~/.ti/extensions/*.ts` 注册自定义工具，参考 pi）；skills（SKILL.md 渐进披露）；cost 金额统计（内置可配置价格表）；`/init` 生成 AGENTS.md；bash 后台任务；粘贴多行优化；PTC 模式调研（dsh）。
...
- **权限**：循环层加 `beforeToolCall(name, input) → allow | always | deny` 钩子（对齐 pi 的钩子位）；`always` 记入内存集合
...
| v0.3 | F3 权限 + F4 compact + F5 历史 + F6 token 累计 | 验收标准全过 |
