...
实现方案写进 `DESIGN.md` 对应功能那一节，等人点头再写代码。不另开一份按版本号命名的方案页。
...
| 0.0.3 | session 持久化与恢复 | 代码在工作区，未提交、未发 |
| 0.0.4 | `/compact` 上下文压缩 | 未做 |
| 0.0.5 | 输入历史落盘 | 未做 |
| 0.0.6 | skills | 未做 |
| 0.0.7 | 冒烟测试 | 未做 |
...
先把 0.0.3（session）CR 完、提交，再决定发不发。下一号是 0.0.4（`/compact`），仍先谈需求再写方案。
